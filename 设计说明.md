...
| 调度 | 基于 `priority` 实现优先级队列，通过消费者组实现集群式调度；按供应商 SLA 做动态流控与隔离 |
...
说明：下图展示的是第一版系统的目标架构。当前 MVP 实现中，重试调度和死信处理合并在 `NotificationDispatcher` 内，可观测层先以日志和任务状态为主，完整指标、告警和死信回放属于后续演进。
